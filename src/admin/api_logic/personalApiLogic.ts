// src/admin/api_logic/personalApiLogic.ts
...
export async function listItem(c:Context) {
    ...
    try {
        ...
        const result = await db.select().from(articleCategorySchema).all();
...
export async function createItem(c:Context) {
    ...
    try {
        // 查询特定用户名的管理员用户，并比较密码（假设密码存储为明文，实际应用中应使用哈希）
        const result = await db.select().from(articleCategorySchema).get();
        return await sendSuccessResponse(result);
    } catch (error) {
        // 捕获任何查询执行期间发生的错误
        console.error('Error executing query:', error);
...
export async function detailItem(c:Context) {
    try {
        ...
        const db = drizzle(c.env.DB); 
        const result = await db.select().from(articleCategorySchema).where(eq(articleCategorySchema.id, id)).get();
        return await sendSuccessResponse(result);
    } catch (error) {
        // 捕获任何查询执行期间发生的错误
        console.error('Error executing query:', error);
