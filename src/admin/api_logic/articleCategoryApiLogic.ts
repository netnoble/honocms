// src/admin/api_logic/articleCategoryApiLogic.ts
...
export async function createItem(c:Context) {
    try {
        ...
        const result = await db.insert(articleCategorySchema).values({
            name: params.name,
...
export async function updateItem(c:Context) {
    try {
        ...
        const db = drizzle(c.env.DB); 
        const result = await db.update(articleCategorySchema)
            .set({
                name: params.name,
                parent_id: params.parent_id,
                remark: params.remark,
            ...
            .where(eq(articleCategorySchema.id, params.id));
        if(result){
            return await sendSuccessResponse(result);
        }
